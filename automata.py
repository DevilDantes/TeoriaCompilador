...
class Lexer:
    ...
    def analizar(self):
        ...
        while self.pos < len(self.texto):
            ...
            if c.isdigit() or c == ".":
                numero += c
            elif c == "-" and not numero and (not self.tokens or self.tokens[-1].tipo in ["OPERADOR", "PAREN_IZQ"]):
                numero += c
            else:
                if numero:
                    if numero == "-": 
...
class CompiladorApp:
    ...
    def layout(self):
        ...
        ttk.Label(top_frame, text="Ingrese la expresión matemática:").pack(side="left")
        ...
        scrollbar_y = ttk.Scrollbar(canvas_frame,
                                     orient="vertical",
                                     command=self.canvas.yview)

        scrollbar_x = ttk.Scrollbar(canvas_frame,
                                     orient="horizontal",
        ...
        self.label_resultado = ttk.Label(bottom_frame,
                                         text="Resultado: ---")
    ...
    def procesar(self):
        try:
            ...
            evaluador = Evaluador()
            self.resultado = evaluador.evaluar(self.arbol)
    ...
    def calcular_posiciones(self, nodo, profundidad, x_actual, posiciones):
        ...
        ancho_izq = self.calcular_posiciones(
            nodo.izq, profundidad+1, x_actual, posiciones)

        if ancho_izq == 0:
            ancho_izq = 1

        ancho_der = self.calcular_posiciones(
            nodo.der, profundidad+1,
            x_actual + ancho_izq * espacio_h,
            posiciones)
